"use client";
// src/components/admin/NewProductForm.tsx
// Formulaire de création de produit monochrome et réactif
...
export default function NewProductForm({ collections }: Props) {
  ...
  const [imageUrl, setImageUrl] = useState("/images/feminine/IMG_3416.JPG.jpeg");
  const [stock, setStock] = useState(20);
  const [collectionId, setCollectionId] = useState(collections[0]?.id || "col-normale");
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Fallback: convert to base64 Data URL so the app continues working locally in demo mode
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn("Product creation API call failed:", err);
      // Demo fallback: if the database is offline, simulate a successful creation
      if (err.message?.includes("connection") || err.message?.includes("serveur") || err.message?.includes("fetch")) {
        setSuccess(true);
        setError("Note: Base de données injoignable. Produit simulé avec succès en mode démo !");
        setTimeout(() => {
          router.push("/admin/produits");
        }, 2000);
      } else {
    ...
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      {success && (
        <div style={{ padding: "12px 16px", background: "rgba(52,211,153,0.08)", border: "1px solid rgba(52,211,153,0.2)", color: "#34d399", fontSize: "13px" }}>
          ✓ {error ? error : "Produit créé avec succès ! Redirection..."}
        </div>
      )}
